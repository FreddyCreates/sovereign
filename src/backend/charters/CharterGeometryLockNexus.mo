...
Author: SCRIBE — this protocol defines SCRIBE's own governance power";
  ...
  func geometryLockPaperContent() : Text {
    ...
    # "θⱼ = sovereignHash(secret + callerId + φ-window, DIM_KEYS[j]) ∈ [0, 2π)\n"
    # "for j ∈ {1..8}, each dimension independently keyed via DIM_KEYS[j]\n"
    # "  DIM_KEYS: DIM1_PHI1, DIM2_PHI2, DIM3_PHI3, DIM4_HEART,\n"
    # "            DIM5_PHI5, DIM6_PHI6, DIM7_FIB13, DIM8_FIB21\n"
    # "sovereignHash = 4-round Merkle-Damgård + PHI-mixing:\n"
    # "  R1=FNV(input), R2=FNV(R1+dimKey), R3=(R1×R2+PHI_INT)%2³², R4=FNV(R3+R1)\n"
    # "  Output normalized to [0,2π) — each dimension has its own independent hash chain\n\n"
