...
import { createActorClass } from './pocket-ic-actor.js';
import { SubnetType, } from './pocket-ic-types.js';
import { MANAGEMENT_CANISTER_ID, decodeCreateCanisterResponse, encodeCreateCanisterRequest, encodeInstallCodeRequest, encodeStartCanisterRequest, encodeUpdateCanisterSettingsRequest, } from './management-canister.js';
import { createDeferredActorClass, } from './pocket-ic-deferred-actor.js';
/**
...
export class PocketIc {
     ...
     * ```
     */
    async createCanister({ sender = Principal.anonymous(), cycles = 1000000000000000000n, controllers, computeAllocation, freezingThreshold, memoryAllocation, reservedCyclesLimit, targetCanisterId, targetSubnetId, } = {}) {
     ...
     * ```
     */
    async startCanister({ canisterId, sender = Principal.anonymous(), targetSubnetId, }) {
     ...
     * ```
     */
    async stopCanister({ canisterId, sender = Principal.anonymous(), targetSubnetId, }) {
        const payload = encodeStartCanisterRequest({
            canister_id: canisterId,
        });
        await this.client.updateCall({
     ...
     * ```
     */
    async installCode({ arg = new Uint8Array(), sender = Principal.anonymous(), canisterId, wasm, targetSubnetId, }) {
     ...
     * ```
     */
    async reinstallCode({ sender = Principal.anonymous(), arg = new Uint8Array(), canisterId, wasm, }) {
        if (typeof wasm === 'string') {
            wasm = await readFileAsBytes(wasm);
        }
        const payload = encodeInstallCodeRequest({
     ...
     * ```
     */
    async upgradeCanister({ sender = Principal.anonymous(), arg = new Uint8Array(), canisterId, wasm, }) {
        if (typeof wasm === 'string') {
            wasm = await readFileAsBytes(wasm);
        }
        const payload = encodeInstallCodeRequest({
     ...
     * ```
     */
    async updateCanisterSettings({ canisterId, computeAllocation, controllers, freezingThreshold, memoryAllocation, reservedCyclesLimit, sender = Principal.anonymous(), }) {
    ...
    createActor(interfaceFactory, canisterId) {
        const Actor = createActorClass(interfaceFactory, canisterId, this.client);
        return new Actor();
    }
    /**
    ...
    createDeferredActor(interfaceFactory, canisterId) {
        const DeferredActor = createDeferredActorClass(interfaceFactory, canisterId, this.client);
     ...
     * ```
     */
    async queryCall({ canisterId, method, arg = new Uint8Array(), sender = Principal.anonymous(), targetSubnetId, }) {
     ...
     * ```
     */
    async updateCall({ canisterId, method, arg = new Uint8Array(), sender = Principal.anonymous(), targetSubnetId, }) {
        const res = await this.client.updateCall({
            canisterId,
            method,
            payload: new Uint8Array(arg),
